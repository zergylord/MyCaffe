...
//BEGIN: my function
template <typename Dtype>
void caffe_gpu_cond_scal(const int N, const Dtype alpha, const Dtype* x, Dtype* y);
template <typename Dtype>
void caffe_gpu_cond_add(const int N, const Dtype* a, const Dtype* x, Dtype* y);
template <typename Dtype>
void caffe_gpu_cond_add_scalar(const int N, const Dtype a, const Dtype* x, Dtype* y);
//END: my function
template <typename Dtype>
...
void caffe_gpu_##name<double>(const int n, const double* x, double* y) { \
  ...
  name##_kernel<double><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n, x, y); \
} /* Guess I don't need to declare global kernels...
template <> \
void caffe_gpu_##name<float>(const int n, const float alpha,const float* x, float* y) { \
   \
  name##_kernel<float><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n, alpha,x, y); \
} \
template <> \
void caffe_gpu_##name<double>(const int n, const double alpha, const double* x, double* y) { \
   \
  name##_kernel<double><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n,alpha, x, y); \
}\
template <> \
void caffe_gpu_##name<float>(const int n, const float* a,const float* x, float* y) { \
   \
  name##_kernel<float><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n, a,x, y); \
} \
template <> \
void caffe_gpu_##name<double>(const int n, const double* a, const double* x, double* y) { \
  \
  name##_kernel<double><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n,a, x, y); \
}
