...
Dtype QNetLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int num = bottom[0]->num();
    ...
    caffe_gpu_cond_add(count,
            difference_.gpu_data(),
            bottom[0]->gpu_data(),
            difference_.mutable_gpu_data());
    caffe_gpu_add_scalar(count,Dtype(1),difference_.mutable_gpu_data());//undo the first +1
  Dtype loss = 1337;
  return loss;
}
...
void QNetLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  int count = (*bottom)[0]->count();
  int num = (*bottom)[0]->num();
  caffe_gpu_copy(count,difference_.gpu_data(),(*bottom)[0]->mutable_cpu_diff());
}*/
