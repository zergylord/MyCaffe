...
//Uncomment and seperate function if you wanna merge
/*
template <typename Dtype>
...
// Interactive Solve: 1 call = 1 iteration
template <typename Dtype>
void Solver<Dtype>::Solve(const char* resume_file) {
  //LOG(INFO) << "Solving " << net_->name();
  if (iter_ == 0) {
      LOG(INFO) << "babys first iteration!";
      Caffe::set_mode(Caffe::Brew(param_.solver_mode()));
      if (param_.solver_mode() && param_.has_device_id()) {
        Caffe::SetDevice(param_.device_id());
      }
...
void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::GPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      // Compute the value to history, and then copy them to the blob's diff.
      Dtype rms_gamma = this->param_.rms_gamma();
      Dtype not_rms_gamma = Dtype(1) - this->param_.rms_gamma();
      Dtype local_rate = rate * net_params_lr[param_id];
      Dtype local_decay = weight_decay * net_params_weight_decay[param_id];
      //BEGIN crazy edits
      //BEGIN RMS UPDATE-----------------------------------
      //get the sign of gpu_diff elements
      //only update rms elements for indices where cur_sign != 0? yup, cause fuck the Qnet architecutre
      //maybe: check if all grads are zero; if so, don't update rms. NO
      //caffe_gpu_equal takes an array and sets the return array to a mask of those indices that are 0
      //LOG(INFO) << "RMS grad square";
      //store grad^2 in temp
      caffe_gpu_powx(net_params[param_id]->count(),
           net_params[param_id]->gpu_diff(),
           (Dtype) 2,
           history_[param_id]->mutable_gpu_data());//use as temp var 
 /*     caffe_gpu_scal(net_params[param_id]->count(),
           not_rms_gamma,
           history_[param_id]->mutable_gpu_data());
      //LOG(INFO) << "RMS 1-gamma*grad^2 + gamma*rms";
      // (1-gamma)*(grad^2)+gamma*rms
      caffe_gpu_cond_scal(net_params[param_id]->count(),
           rms_gamma,
           net_params[param_id]->gpu_diff(),//if this is zero, don't scale
           net_params[param_id]->mutable_gpu_rms()); 
      caffe_gpu_cond_add(net_params[param_id]->count(),
           history_[param_id]->mutable_gpu_data(),
           net_params[param_id]->gpu_diff(),//if this is zero, don't add
           net_params[param_id]->mutable_gpu_rms()); 
 */     caffe_gpu_axpby(net_params[param_id]->count(),
           not_rms_gamma,
           history_[param_id]->gpu_data(),//use as temp var 
           rms_gamma,
           net_params[param_id]->mutable_gpu_rms());
      //add gpu_diff to rms
       //LOG(INFO) << net_params[param_id]->cpu_rms()[0];
      //END RMS UPDATE-------------------------------------
      //LOG(INFO) << "sqrt rms";
      //take sqrt of rms store in a temp (currently hist)
      caffe_gpu_powx(net_params[param_id]->count(),
              net_params[param_id]->gpu_rms(),
              (Dtype) .5,
              history_[param_id]->mutable_gpu_data()); 
      //LOG(INFO) << "divide by sqrt rms";
      //gpu_diff/sqrt(rms)   penis
      caffe_gpu_div(net_params[param_id]->count(),
           net_params[param_id]->gpu_diff(),   
           history_[param_id]->gpu_data(),//sqrt(rms) stored here
           history_[param_id]->mutable_gpu_data());
      //LOG(INFO) << "scale by lr";
      //mult by learn rate
      caffe_gpu_scal(net_params[param_id]->count(),
           local_rate,
           history_[param_id]->mutable_gpu_data());           
