...
void Blob<Dtype>::Reshape(const int num, const int channels, const int height,
  ...
  const int temp_count = num*channels*height*width;
  if (count_) {
    data_.reset(new SyncedMemory(count_ * sizeof(Dtype)));
    diff_.reset(new SyncedMemory(count_ * sizeof(Dtype)));
    rms_.reset(new SyncedMemory(count_ * sizeof(Dtype)));
    caffe_gpu_add_scalar(temp_count,
            (Dtype) 1000,
            mutable_gpu_rms());
  } else {
    data_.reset(reinterpret_cast<SyncedMemory*>(NULL));
    diff_.reset(reinterpret_cast<SyncedMemory*>(NULL));
    rms_.reset(reinterpret_cast<SyncedMemory*>(NULL));
  }
}
...
//BEGIN edits
template <typename Dtype>
const Dtype* Blob<Dtype>::gpu_rms() const {
  CHECK(rms_);
  return (const Dtype*)rms_->gpu_data();
}

template <typename Dtype>
Dtype* Blob<Dtype>::mutable_gpu_rms() {
  CHECK(rms_);
  return reinterpret_cast<Dtype*>(rms_->mutable_gpu_data());
}

template <typename Dtype>
const Dtype* Blob<Dtype>::cpu_rms() const {
  CHECK(rms_);
  return (const Dtype*)rms_->cpu_data();
}
template <typename Dtype>
Dtype* Blob<Dtype>::mutable_cpu_rms() {
  CHECK(rms_);
  return reinterpret_cast<Dtype*>(rms_->mutable_cpu_data());
}
//END edits
template <typename Dtype>
...
void Blob<Dtype>::FromProto(const BlobProto& proto) {
  ...
  //Dtype* rms_vec = mutable_gpu_rms();//do I need a cpu rms?
  for (int i = 0; i < count_; ++i) {
    data_vec[i] = proto.data(i);
    //rms_vec[i] = proto.rms(i);
  }
...
void Blob<Dtype>::ToProto(BlobProto* proto, bool write_diff) const {
  ...
  //const Dtype* rms_vec = gpu_rms();//do I need a cpu rms?
  for (int i = 0; i < count_; ++i) {
    proto->add_data(data_vec[i]);
    //proto->add_rms(rms_vec[i]);
  }
  if (write_diff) {
